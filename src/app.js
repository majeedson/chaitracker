...
async function renderLogin(root, supabase) {
      ...
      <section class="login-card">
        ...
        <div id="login-picker">
          ...
          <div id="onboarding-start" hidden>
            <div class="onboard-invite"><div class="feature-icon">${icon('user',22)}</div><div><strong>Welcome to CafeTracker</strong><span>Complete your profile and create your private PIN.</span></div></div>
            <button id="start-onboarding" type="button" class="primary full">Complete setup <span>→</span></button>
          </div>
  ...
  function renderOnboarding(){
    ...
    const saveStep=()=>{
      ...
      if(step===0)Object.assign(state,{full_legal_name:get('#ob-full'),date_of_birth:get('#ob-dob'),nationality:get('#ob-nationality'),father_name:get('#ob-father'),mobile_phone:get('#ob-mobile'),home_address:get('#ob-address')});
      if(step===1)Object.assign(state,{emergency_contact_name:get('#ob-ec-name'),emergency_contact_relation:get('#ob-ec-rel'),emergency_contact_phone:get('#ob-ec-phone')});
      if(step===2)Object.assign(state,{identity_type:get('#ob-id-type'),identity_number:get('#ob-id-number')});
    };
    const validateStep=()=>{
      const err=onboardingShell.querySelector('#ob-error');let msg='';
      if(step===0&&['#ob-full','#ob-dob','#ob-nationality','#ob-father','#ob-mobile','#ob-address'].some(id=>!onboardingShell.querySelector(id).value.trim()))msg='Please complete all personal details.';
      if(step===1&&['#ob-ec-name','#ob-ec-rel','#ob-ec-phone'].some(id=>!onboardingShell.querySelector(id).value.trim()))msg='Please complete the emergency contact details.';
...
async function renderPurchaseOrders(view,supabase,profile){
  ...
  const countPane=view.querySelector('#tonightStockPane'),orderPane=view.querySelector('#tomorrowOrderSection'),workflowBtns=[...view.querySelectorAll('[data-stock-view]')];
  const switchStockView=mode=>{workflowBtns.forEach(b=>b.classList.toggle('active',b.dataset.stockView===mode));countPane.hidden=mode!=='count';orderPane.hidden=mode!=='order';if(mode==='order'&&!orderPane.dataset.loaded)loadOrders();window.scrollTo({top:0,behavior:'smooth'});};
  workflowBtns.forEach(b=>b.onclick=()=>switchStockView(b.dataset.stockView));
...
async function renderStock(view,supabase,profile){
  ...
  const loadOrders=async()=>{
    ...
    const rows=(data||[]).filter(x=>!['VENDOR_MANAGED','CONTROLLED'].includes(x.order_strategy));
    ...
    const state=new Map(rows.map(x=>[String(x.item_id),{checked:Number(x.suggested_qty||0)>0,qty:Number(x.suggested_qty||0)>0?Number(x.suggested_qty):''}]));
    section.insertAdjacentHTML('afterbegin','<div class="order-group-toggle"><button type="button" data-mode="vendor" class="active">Vendor</button><button type="button" data-mode="category">Category</button></div>');
    const renderGroups=()=>{
      box.querySelectorAll('.order-suggestion-row').forEach(r=>state.set(String(r.dataset.id),{checked:r.querySelector('.order-include').checked,qty:r.querySelector('.order-qty').value}));
      const groups=new Map();rows.forEach(x=>{const k=groupMode==='vendor'?(x.vendor_name||'Unassigned'):(x.category_name||'Other');if(!groups.has(k))groups.set(k,[]);groups.get(k).push(x);});
      box.innerHTML=[...groups].map(([name,list])=>`<details class="order-category-group" data-category="${escapeHtml(name)}"><summary><span><strong>${escapeHtml(name)}</strong><small>${list.filter(x=>state.get(String(x.item_id))?.checked).length} selected</small></span><button type="button" class="order-add-item" aria-label="Add item">+</button></summary><div class="order-category-body">${list.map(x=>{const st=state.get(String(x.item_id))||{checked:false,qty:''};return `<div class="order-suggestion-row" data-id="${x.item_id}" data-vendor="${escapeHtml(x.vendor_name||'Unassigned')}" data-name="${escapeHtml(x.item_name)}" data-unit="${escapeHtml(x.order_unit||'')}"><input class="order-include" type="checkbox" ${st.checked?'checked':''}><span><strong>${escapeHtml(x.item_name)}</strong><small>Stock ${Number(x.current_stock||0)}</small></span><input class="order-qty" type="number" min="0" step="0.01" value="${st.qty}" placeholder="0"><em>${escapeHtml(x.order_unit||'')}</em></div>`;}).join('')}<div class="order-category-actions"><button type="button" class="summary-add category-message">Generate message</button></div><div class="category-add-panel" hidden><label>Find item<input class="category-add-search" type="search" placeholder="Search all items…"></label><div class="category-add-results"></div></div><div class="category-order-preview" hidden><textarea readonly></textarea><button type="button" class="summary-add copy-category-message">Copy</button></div></div></details>`).join('');
      bindGroups();
    };
    ...
    const bindGroups=()=>view.querySelectorAll('.order-category-group').forEach(g=>{
      ...
      search.oninput=()=>{const q=search.value.trim().toLowerCase();if(q.length<1){results.innerHTML='';return;}const existing=new Set([...box.querySelectorAll('.order-suggestion-row')].map(r=>String(r.dataset.id)));const found=(catalog||[]).filter(i=>!existing.has(String(i.id))&&i.name.toLowerCase().includes(q)).slice(0,8);results.innerHTML=found.map(i=>`<button type="button" class="category-add-result" data-id="${i.id}" data-name="${escapeHtml(i.name)}" data-unit="${escapeHtml(i.unit||'')}"><span>${escapeHtml(i.name)}</span><small>${escapeHtml(i.categories?.name||'Other')}</small></button>`).join('')||'<small>No matching item.</small>';results.querySelectorAll('.category-add-result').forEach(btn=>btn.onclick=()=>{const body=g.querySelector('.order-category-body'),actions=g.querySelector('.order-category-actions');const row=document.createElement('div');row.className='order-suggestion-row';row.dataset.id=btn.dataset.id;row.dataset.vendor='Unassigned';row.dataset.name=btn.dataset.name;row.dataset.unit=btn.dataset.unit;row.innerHTML='<input class="order-include" type="checkbox" checked><span><strong>'+escapeHtml(btn.dataset.name)+'</strong><small>Added manually</small></span><input class="order-qty" type="number" min="0" step="0.01" placeholder="0"><em>'+escapeHtml(btn.dataset.unit)+'</em>';body.insertBefore(row,actions);search.value='';results.innerHTML='';});};
      ...
      g.querySelector('.copy-category-message').onclick=async()=>{const ta=g.querySelector('textarea');try{await navigator.clipboard.writeText(ta.value);}catch{ta.select();}};
    });
  ...
  const save=view.querySelector('#saveTonightStock');if(save)save.onclick=async()=>{
    ...
    show('Tonight’s stock saved. Tomorrow’s order is ready to review.','success');save.textContent='Saved ✓';await loadOrders();switchStockView('order');
  };
