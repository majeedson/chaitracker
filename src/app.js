...
async function renderStock(view,supabase,profile){
  ...
  const loadOrders=async()=>{
    ...
    box.innerHTML=[...groups].map(([category,list])=>`<div class="order-category-group" data-category="${escapeHtml(category)}"><div class="section-heading"><h4>${escapeHtml(category)}</h4><button type="button" class="summary-add category-message">Generate message</button></div>${list.map(x=>`<div class="order-suggestion-row" data-vendor="${escapeHtml(x.vendor_name||'Unassigned')}" data-name="${escapeHtml(x.item_name)}" data-unit="${escapeHtml(x.order_unit||'')}"><input class="order-include" type="checkbox" checked><span><strong>${escapeHtml(x.item_name)}</strong><small>Stock ${Number(x.current_stock||0)} · Suggested ${Number(x.suggested_qty||0)} · ${escapeHtml(x.suggestion_basis||'')}</small></span><input class="order-qty" type="number" min="0" step="0.01" value="${Number(x.suggested_qty||0)}"><em>${escapeHtml(x.order_unit||'')}</em></div>`).join('')}<div class="category-order-preview" hidden><textarea readonly></textarea><button type="button" class="summary-add copy-category-message">Copy</button></div></div>`).join('');
    ...
    view.querySelectorAll('.order-category-group').forEach(g=>{g.querySelector('.category-message').onclick=()=>{const p=g.querySelector('.category-order-preview'),ta=p.querySelector('textarea');ta.value=buildMessage(g,g.dataset.category+' order');p.hidden=false;ta.style.height='auto';ta.style.height=Math.min(ta.scrollHeight,300)+'px';};g.querySelector('.copy-category-message').onclick=async()=>{const ta=g.querySelector('textarea');try{await navigator.clipboard.writeText(ta.value);}catch{ta.select();}};});
