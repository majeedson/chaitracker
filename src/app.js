...
async function renderStock(view,supabase,profile){
  ...
  const loadOrders=async()=>{
    ...
    const groups=new Map();rows.forEach(x=>{const k=x.category_name||'Other';if(!groups.has(k))groups.set(k,[]);groups.get(k).push(x);});
    box.innerHTML=[...groups].map(([category,list])=>`<details class="order-category-group" data-category="${escapeHtml(category)}"><summary><span><strong>${escapeHtml(category)}</strong><small>${list.filter(x=>Number(x.suggested_qty)>0).length} suggested</small></span><button type="button" class="order-add-item" aria-label="Add item to ${escapeHtml(category)}">+</button></summary><div class="order-category-body">${list.map(x=>{const suggested=Number(x.suggested_qty||0)>0;return `<div class="order-suggestion-row" data-id="${x.item_id}" data-vendor="${escapeHtml(x.vendor_name||'Unassigned')}" data-name="${escapeHtml(x.item_name)}" data-unit="${escapeHtml(x.order_unit||'')}"><input class="order-include" type="checkbox" ${suggested?'checked':''}><span><strong>${escapeHtml(x.item_name)}</strong><small>Stock ${Number(x.current_stock||0)} · ${suggested?'Suggested '+Number(x.suggested_qty):'Not suggested'}${x.suggestion_basis?' · '+escapeHtml(x.suggestion_basis):''}</small></span><input class="order-qty" type="number" min="0" step="0.01" value="${suggested?Number(x.suggested_qty):''}" placeholder="0"><em>${escapeHtml(x.order_unit||'')}</em></div>`;}).join('')}<div class="order-category-actions"><button type="button" class="summary-add category-message">Generate message</button></div><div class="category-add-panel" hidden><label>Find item<input class="category-add-search" type="search" placeholder="Search all items…"></label><div class="category-add-results"></div></div><div class="category-order-preview" hidden><textarea readonly></textarea><button type="button" class="summary-add copy-category-message">Copy</button></div></div></details>`).join('');
    ...
    view.querySelectorAll('.order-category-group').forEach(g=>{
