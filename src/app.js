...
async function renderStock(view,supabase,profile){
  ...
  const loadOrders=async()=>{
    ...
    const bindGroups=()=>view.querySelectorAll('.order-category-group').forEach(g=>{
      g.querySelectorAll('.order-remove-item').forEach(btn=>btn.onclick=e=>{e.preventDefault();e.stopPropagation();const row=btn.closest('.order-suggestion-row'),id=String(row.dataset.id),prev=state.get(id)||{};state.set(id,{...prev,checked:false,removed:true});renderGroups();});
