...
async function renderStock(view,supabase,profile){
  ...
  const [{data:rows,error},{data:day}]=await Promise.all([supabase.from('current_stock').select('item_id,item_name,unit,minimum_stock,reorder_qty,count_now,business_date').eq('outlet_id',outletId).order('item_name'),supabase.from('business_days').select('stock_status').eq('outlet_id',outletId).eq('business_date',businessDate).maybeSingle()]);
  if(error){view.innerHTML='<span class="eyebrow">Stock</span><h2>Stock unavailable</h2><p class="form-error">'+escapeHtml(error.message)+'</p>';return;}
  const latest=new Map();(rows||[]).forEach(r=>{if(!latest.has(r.item_id))latest.set(r.item_id,r);});const items=[...latest.values()],closed=day?.stock_status==='CLOSED',canClose=profile.access_class==='ADMIN'||['Manager','Ops Manager'].includes(profile.role);
